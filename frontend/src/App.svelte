<script>
  ...
  async function handle_submission(results, text = "") {
    ...
    if (response.ok) {
      ...
      if (logging) {
        ...
        const response2 = await fetch(`${window.location.hostname}:42032/append_user_log`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  async function get_session_id_for_user() {
    ...
    if (logging) {
      let url_log = `${window.location.hostname}:42032/create_user_log`;
  ...
  async function request_handler(request_url, request_body, init = false, method = "", image_upload = false, query = "", video_image_id = 0, is_sorted = false, reset = false) {
    ...
    try {
      const url = `${window.location.hostname}:42032/send_request_to_service`;
  ...
  async function bayesUpdate() {
    ...
    const url = `${window.location.hostname}:42032/bayes`;
  ...
  async function traverse_states(direction) {
    ...
    let url = `${window.location.hostname}:42032/${direction === -1 ? 'back' : 'forward'}`;

<script>
  // @ts-nocheck
    ...
    // let users = ['user01', 'user02', 'user03', 'user04', 'user05', 'user06', 'user07', 'user08', 'user09', 'user10', 'user11', 'user12', 'user13', 'user14', 'user15', 'user16', 'user17', 'user18', 'user19', 'user20', 'user21', 'user22', 'user23', 'user24', 'user25', 'user26', 'user27', 'user28', 'user29', 'user30'];
    let users = Array.from({length: 30}, (_, i) => `user${String(i+1).padStart(2, '0')}`);
  
    let passwords = new Array(30).fill('NDBI045');
    ...
    async function handle_submission(results, text=''){
      ...
      if (response.ok) {
        ...
        if (res['submission'] == "CORRECT") {
          correct_send_text = "Submission was correct!";
        }
        ...
        let request = await fetch("http://hmon.ms.mff.cuni.cz:8443/api/v2/log/result/" + eval_id + "?session=" + session_id, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    async function get_session_id_for_user(){
      ...
      let i = "http://hmon.ms.mff.cuni.cz:8443/api/v2/client/evaluation/list?session="+session_id;
    ...
    async function label_click(event){
      ...
      if (filtered_lables.includes(clickedId)){
        ...
        if (filtered_lables.length > 0){
          for (const [key] of Object.entries(temp_items)) {
  ...
  <main>
    <div class='viewbox'>
      <div class='top-menu'>
        <a class="title" href="https://docs.google.com/forms/d/e/1FAIpQLSdh0YE6N3QKcTQjlXIoWgOIhFLanBedIExB98WRC9clvBvy5w/viewform?usp=sf_link" title="Dotazník" target="_blank">Video Hunter</a>
        ...
        <div class ="top-menu-item top-negative-offset3" >
          ...
          <span class="top-menu-item" id="target_text">{correct_send_text}</span>
      </div>
